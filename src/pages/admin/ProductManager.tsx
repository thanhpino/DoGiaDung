import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { toast } from 'react-hot-toast';
import { Trash2, Plus, X, Edit, Save, RotateCcw, Search, Package } from 'lucide-react';

// Danh sách danh mục chuẩn
const CATEGORIES = [
  { id: 'Kitchen', label: 'Nhà Bếp & Nấu Ăn' },
  { id: 'Cleaning', label: 'Dọn Dẹp & Vệ Sinh' },
  { id: 'SmartHome', label: 'Nhà Thông Minh' },
  { id: 'Cooling', label: 'Điện Máy & Làm Mát' },
  { id: 'Lighting', label: 'Đèn & Chiếu Sáng' },
  { id: 'Bedroom', label: 'Phòng Ngủ (Chăn Ga)' },
  { id: 'Bathroom', label: 'Phòng Tắm' },
  { id: 'Decor', label: 'Trang Trí (Decor)' },
  { id: 'Gadget', label: 'Công Nghệ & Phụ Kiện' },
  { id: 'Health', label: 'Sức Khỏe & Thể Thao' },
  { id: 'Beauty', label: 'Làm Đẹp' },
  { id: 'Utility', label: 'Tiện Ích Đời Sống' },
  { id: 'Personal', label: 'Đồ Dùng Cá Nhân' }
];

export const ProductManager = () => {
  const [products, setProducts] = useState<any[]>([]);
  const [filteredProducts, setFilteredProducts] = useState<any[]>([]);
  const [showForm, setShowForm] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');
  
  // Đang sửa sản phẩm nào (null = không sửa, đang thêm mới)
  const [editingId, setEditingId] = useState<number | null>(null);

  // Form data
  const [formData, setFormData] = useState({
    name: '', price: '', category: 'Kitchen', img: '', description: ''
  });

  // 1. LẤY DỮ LIỆU
  const fetchProducts = async () => {
    try {
        const res = await axios.get(`${import.meta.env.VITE_API_URL}/products?limit=1000`);
        const data = res.data.data || (Array.isArray(res.data) ? res.data : []);
        setProducts(data);
        setFilteredProducts(data);
    } catch (err) { console.error(err); }
  };

  useEffect(() => { fetchProducts(); }, []);

  // Filter search
  useEffect(() => {
    const lower = searchTerm.toLowerCase();
    const filtered = products.filter(p => 
        p.name.toLowerCase().includes(lower) || 
        p.category.toLowerCase().includes(lower) ||
        p.id.toString().includes(lower)
    );
    setFilteredProducts(filtered);
  }, [searchTerm, products]);

  // 2. XỬ LÝ SUBMIT
  const handleSubmit = async (e: React.FormEvent) => {
      e.preventDefault();
      try {
          if (editingId) {
              await axios.put(`${import.meta.env.VITE_API_URL}/api/products/${editingId}`, formData);
              toast.success("Cập nhật thành công!");
          } else {
              await axios.post(`${import.meta.env.VITE_API_URL}/api/products`, formData);
              toast.success("Thêm mới thành công!");
          }
          
          resetForm();
          fetchProducts();
      } catch (err) { toast.error("Có lỗi xảy ra"); }
  };

  // 3. CHUẨN BỊ FORM ĐỂ SỬA
  const handleEditClick = (product: any) => {
      setEditingId(product.id);
      setFormData({
          name: product.name,
          price: product.price,
          category: product.category,
          img: product.image_url || product.img,
          description: product.description || ''
      });
      setShowForm(true);
      window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  // 4. XÓA SẢN PHẨM
  const handleDelete = async (id: number) => {
      if(!window.confirm("Bạn chắc chắn muốn xóa sản phẩm này?")) return;
      try {
          await axios.delete(`${import.meta.env.VITE_API_URL}/api/products/${id}`);
          toast.success("Đã xóa sản phẩm!");
          fetchProducts();
      } catch (err) { toast.error("Lỗi xóa sản phẩm"); }
  };

  // 5. RESET FORM
  const resetForm = () => {
      setShowForm(false);
      setEditingId(null);
      setFormData({ name: '', price: '', category: 'Kitchen', img: '', description: '' });
  };

  return (
    <div className="animate-fade-in">
      {/* HEADER & SEARCH */}
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
                <Package className="text-orange-600"/> Quản Lý Sản Phẩm
            </h2>
            <p className="text-sm text-gray-500">Tổng cộng: {products.length} sản phẩm</p>
          </div>

          <div className="flex gap-3 w-full md:w-auto">
              <div className="relative flex-1 md:w-64">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={18}/>
                  <input 
                    type="text" 
                    placeholder="Tìm tên, ID, danh mục..." 
                    className="w-full pl-10 pr-4 py-2 rounded-lg border border-gray-200 focus:border-orange-500 outline-none transition"
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                  />
              </div>
              <button 
                onClick={() => {
                    resetForm();
                    setShowForm(!showForm);
                }} 
                className="bg-orange-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-orange-700 font-bold shadow-sm whitespace-nowrap"
              >
                  {showForm && !editingId ? <X size={20}/> : <Plus size={20}/>} 
                  {showForm && !editingId ? 'Đóng' : 'Thêm Mới'}
              </button>
          </div>
      </div>

      {/* --- FORM NHẬP LIỆU  --- */}
      {showForm && (
          <div className="bg-white p-6 rounded-2xl shadow-lg border border-orange-200 mb-8 animate-slide-in relative overflow-hidden">
              <div className={`absolute top-0 left-0 px-4 py-1 rounded-br-xl font-bold text-sm ${editingId ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'}`}>
                  {editingId ? `Đang chỉnh sửa ID #${editingId}` : 'Thêm sản phẩm mới'}
              </div>

              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                  <div>
                      <label className="text-xs font-bold text-gray-500 uppercase">Tên sản phẩm</label>
                      <input className="w-full border p-2 rounded-lg focus:border-orange-500 outline-none bg-gray-50 focus:bg-white transition" required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                  </div>
                  <div>
                      <label className="text-xs font-bold text-gray-500 uppercase">Giá bán (VNĐ)</label>
                      <input className="w-full border p-2 rounded-lg focus:border-orange-500 outline-none bg-gray-50 focus:bg-white transition" type="number" required value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} />
                  </div>
                  <div>
                      <label className="text-xs font-bold text-gray-500 uppercase">Link Ảnh (URL)</label>
                      <input className="w-full border p-2 rounded-lg focus:border-orange-500 outline-none bg-gray-50 focus:bg-white transition" placeholder="/images/..." required value={formData.img} onChange={e => setFormData({...formData, img: e.target.value})} />
                  </div>
                  <div>
                      <label className="text-xs font-bold text-gray-500 uppercase">Danh mục</label>
                      <select 
                        className="w-full border p-2 rounded-lg focus:border-orange-500 outline-none bg-gray-50 focus:bg-white transition cursor-pointer" 
                        value={formData.category} 
                        onChange={e => setFormData({...formData, category: e.target.value})}
                      >
                          {CATEGORIES.map(cat => (
                              <option key={cat.id} value={cat.id}>{cat.label}</option>
                          ))}
                      </select>
                  </div>
                  <div className="md:col-span-2">
                      <label className="text-xs font-bold text-gray-500 uppercase">Mô tả chi tiết</label>
                      <textarea className="w-full border p-2 rounded-lg focus:border-orange-500 outline-none bg-gray-50 focus:bg-white transition" rows={3} value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})}></textarea>
                  </div>
                  
                  <div className="md:col-span-2 flex gap-3 mt-2">
                      <button type="submit" className={`flex-1 text-white py-3 rounded-lg font-bold shadow-md transition flex items-center justify-center gap-2 ${editingId ? 'bg-blue-600 hover:bg-blue-700' : 'bg-green-600 hover:bg-green-700'}`}>
                          <Save size={20}/> {editingId ? 'Cập Nhật' : 'Lưu Sản Phẩm'}
                      </button>
                      {editingId && (
                          <button type="button" onClick={resetForm} className="px-6 border border-gray-300 rounded-lg hover:bg-gray-100 font-bold text-gray-600 flex items-center gap-2">
                              <RotateCcw size={18}/> Hủy
                          </button>
                      )}
                  </div>
              </form>
          </div>
      )}

      {/* --- DANH SÁCH SẢN PHẨM --- */}
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="overflow-x-auto">
            <table className="w-full text-left">
            <thead className="bg-gray-50 text-gray-500 font-medium text-sm border-b border-gray-100">
                <tr>
                <th className="p-4">ID</th>
                <th className="p-4">Ảnh</th>
                <th className="p-4">Tên sản phẩm</th>
                <th className="p-4">Giá</th>
                <th className="p-4">Danh mục</th>
                <th className="p-4 text-center">Hành động</th>
                </tr>
            </thead>
            <tbody className="text-sm text-gray-700">
                {filteredProducts.length > 0 ? (
                    filteredProducts.map((p) => (
                    <tr key={p.id} className="border-b border-gray-50 hover:bg-orange-50/30 transition group">
                        <td className="p-4 font-mono text-gray-400">#{p.id}</td>
                        <td className="p-4">
                            <div className="w-12 h-12 rounded-lg border bg-white p-1 overflow-hidden">
                                <img src={p.image_url || p.img} className="w-full h-full object-contain" alt={p.name} onError={(e: any) => e.target.src = 'https://via.placeholder.com/50'}/>
                            </div>
                        </td>
                        <td className="p-4 font-bold text-gray-800 group-hover:text-orange-600 transition">{p.name}</td>
                        <td className="p-4 text-orange-600 font-bold">{new Intl.NumberFormat('vi-VN').format(p.price)} ₫</td>
                        <td className="p-4">
                            <span className="bg-gray-100 text-gray-600 px-2 py-1 rounded text-xs font-bold border border-gray-200 whitespace-nowrap">
                                {CATEGORIES.find(c => c.id === p.category)?.label || p.category}
                            </span>
                        </td>
                        <td className="p-4">
                            <div className="flex justify-center gap-2 opacity-60 group-hover:opacity-100 transition">
                                <button onClick={() => handleEditClick(p)} className="bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white p-2 rounded-lg transition shadow-sm" title="Sửa">
                                    <Edit size={18}/>
                                </button>
                                <button onClick={() => handleDelete(p.id)} className="bg-red-50 text-red-500 hover:bg-red-500 hover:text-white p-2 rounded-lg transition shadow-sm" title="Xóa">
                                    <Trash2 size={18}/>
                                </button>
                            </div>
                        </td>
                    </tr>
                    ))
                ) : (
                    <tr><td colSpan={6} className="p-12 text-center text-gray-400 flex flex-col items-center justify-center">
                        <Package size={48} className="mb-2 opacity-20"/>
                        Không tìm thấy sản phẩm nào
                    </td></tr>
                )}
            </tbody>
            </table>
        </div>
      </div>
    </div>
  );
};